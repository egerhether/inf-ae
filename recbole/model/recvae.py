...
class CompositePrior(nn.Module):
    def __init__(self,encoder_class, encoder_args,latent_dim, mixture_weights):
        ...
        # init  frozen encoder
        self.encoder_old = encoder_class(*encoder_args)
...
class ShallowEncoder(nn.Module):
    def __init__(self, input_dim, latent_dim, mlp_hidden_sizes):
        super(ShallowEncoder, self).__init__()

        # layer dims : input -> hidden -> latent
        self.layer_dims = [input_dim] + mlp_hidden_sizes
        self.hidden_layers = nn.ModuleList()

        for in_dim, out_dim in zip(self.layer_dims[:-1], self.layer_dims[1:]):
            self.hidden_layers.append(nn.Linear(in_dim, out_dim))

        self.fc_mu = nn.Linear(self.layer_dims[-1], latent_dim)
        self.fc_logvar = nn.Linear(self.layer_dims[-1], latent_dim)
...
class RecVAE(GeneralRecommender, AutoEncoderMixin):
    ...
    def __init__(self, config, dataset):
        ...
        mlp_raw = config["mlp_hidden_size"]
        if isinstance(mlp_raw, str):
            #split returns list; handles single value strings like "400" or "400,400" to use as num_layers and hidden_layer size 
            try:
                self.layer = list(map(int, mlp_raw.strip().split(",")))
            except Exception as e:
                raise ValueError(f"Cannot parse mlp_hidden_size='{mlp_raw}': {e}")
        elif isinstance(mlp_raw, (list, tuple)):
            self.layer = list(mlp_raw)
        elif isinstance(mlp_raw,int):
            self.layer = [mlp_raw]
        else:
            raise ValueError(f"Unrecognized format for mlp_hidden_size: {mlp_raw}")
        ...
        if self.mlp_hidden_size:
            encoder_class = ShallowEncoder 
            encoder_args = (self.n_items, self.latent_dim, self.layer)
        else:
            encoder_class = Encoder
            encoder_args = (self.hidden_dim, self.latent_dim, self.n_items)
